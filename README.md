...
Tab 5: 
Part A: Try to predict `passengers_per_hour` from 7 features. 3 features are binary (yes/no), 1 is a category, 2 are continuous variable, and 1 is a probability feature. Here is where I used sklearn's `LinearRegression()` rather than code my own. 
Part B: After fitting lin regression, I computed residuals `errors = y_test - preds`. These errors were treated as samples from a Normal distribution. The MLE for var when I fixed the mean at zero is average of squared errors (showned by `sigma_squared = np.mean(errors**2)`. This Hw assignment taught me how to use ML to get a predicted outcome/answer and a probabilistic error model. 
